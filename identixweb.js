...
if (0 == mode)
	var path = 'shivani-savani.myshopify.com',
		...
		target = 'http://localhost/appsonrent-apps/order-delivery-date/order-delivery-date_v1.php',
	...
	if (void 0 === document.currentScript) var scid = '';
		...
		var Scripturl = document.currentScript.src,
			...
			scid = res1[1];
	var target =
		'https://app.identixweb.com/' + server_folder + 'order-delivery-date/order-delivery-date_v1.php';
	if ('product' === __st.p)
	...
	var oddSiteFdUrl = 'https://app.identixweb.com/' + server_folder + 'order-delivery-date/';
}
var oddShopInfo = {},
	oddshipping_price = '',
	oddshipping_method = '',
...
if (cart > 0) {
	...
	if (
		...
		('charlescvvonlinest.myshopify.com' == shopName && '2.1.4' == window.jQuery.fn.jquery) ||
		('chandni-demo.myshopify.com' == shopName && '2.2.3' == window.jQuery.fn.jquery)
	)
		scripts.push(oddSiteFdUrl + 'assets/js/jquery-3.4.1-front.min.js'), oddt_loadScripts(scripts);
	else if ('undefined' == typeof jQuery)
		scripts.push(oddSiteFdUrl + 'assets/js/jquery-3.4.1-front.min.js'), oddt_loadScripts(scripts);
	else if ('undefined' != typeof jQuery) {
		var $ = jQuery.noConflict();
		scripts.push(oddSiteFdUrl + 'assets/js/jquery-ui-front-min.js'), oddt_loadScripts(scripts);
	}
...
function oddt_loadScripts(e) {
	...
	t.append(r),
		(r.onload = function (t) {
			void 0 === jQuery.ui && e.push(oddSiteFdUrl + 'assets/js/jquery-ui-front-min.js'),
				e.length
					? oddt_loadScripts(e)
			...
			success: function (e) {
				if (
					...
					if (
						...
						oddJsLoad(oddSiteFdUrl + 'assets/js/product-calendar.min.js', 'oddProdutCalendarJs');
					else if (
						((deliveryDateText =
						...
						if (
							(oddJsLoad(oddSiteFdUrl + 'assets/js/store-pickup.min.js', 'oddStorePickupJs'),
							(local_html = oddresultJson.local_html),
							$(oddAppDiv).length
								? $(oddAppDiv).html(oddresultJson.html)
								: (1 == oddresultJson.cal_type
										? $(oddSelectorCartForm + ',' + oddSelectorCartForm2 + ',' + oddSelectorCartForm3).append(
												'<div class="identixweb-order-delivery"> ' + oddresultJson.html + ' </div>'
										  )
										: $(oddSelectorCartForm + ',' + oddSelectorCartForm2 + ',' + oddSelectorCartForm2).append(
												'<div class="identixweb-order-delivery"> ' + oddresultJson.html + ' </div>'
										  ),
								  $("form[action='/cart/update'] .identixweb-order-delivery").remove()),
							$('body').append('<style>' + oddresultJson.style + '</style>'),
							$(oddSelectorCartForm).length || $(oddSelectorCartForm2).length || 0 == mode)
						)
							if ('1' == oddresultJson.shipping_status) shipping_calender(1);
							else if ('1' == oddresultJson.sp_app_status)
								var r = setInterval(function () {
									'undefined' != typeof store_pickup_calender && (clearInterval(r), store_pickup_calender(r));
								}, 500);
							else if ('1' == oddresultJson.ld_app_status) {
								$('.main_loader').after(local_html),
									$('.main_loader .identixweb-order-delivery-date-local-delivery-preloader').hide(),
									$('.local_loader .identixweb-order-delivery-date-local-delivery-preloader').hide();
								var i = $("form[action*='/cart'],form[action='/cart?locale=en']"),
									...
									'1' == oddresultJson.ld_required_msg_status &&
										($('a[href="/checkout"]').length &&
											$('a[href="/checkout"]').click(function (e) {
												doRequired(e, oddresultJson, d.local_delivery_subheading, i);
											}),
										...
										i.submit(function (e) {}));
							} else $('.identixweb-order-delivery-date-local-delivery-preloader').hide();
					} else
						($(oddSelectorCartForm).length || $(oddSelectorCartForm2).length || 0 == mode) &&
							($(oddAppDiv).length
								? $('.identixweb-order-delivery').html(oddresultJson.shipping_loader)
								: $(oddSelectorCartForm + ',' + oddSelectorCartForm2).append(
										'<div class="identixweb-order-delivery">' + oddresultJson.shipping_loader + '</div>'
								  )),
							shipping_show(0, (oddShippingCalander = oddresultJson.shipping_data));
				}
			},
		});
}
...
function shipping_show(e, t) {
	...
	if (i < 0 && 'pro' == t.account_type);
		...
		if (
			...
			(v = oddt_get_time_slots(t, _, m, t.auto_current_month)),
				...
				'' == v &&
					...
					(o = o + ',' + oddt_get_date_formated(new Date(t.current_time_c), t.date_format))),
		...
		1 == e
			? 1 == t.enable &&
			  1 == t.shipping_status &&
			  ($('.identixweb-order-delivery-date-main').remove(), $('.main_loader').after(t.shipping_html))
			: $(oddAppDiv).length
...
function date_select(e, t, a, r, i) {
	...
	if ($.inArray('7', o) > -1) {
		...
		if ($.inArray(d, s) > -1)
			return void $('.identixweb-order-delivery-date-deliveryRequired')
				.html("Sorry... You can't order for selected date. Please select other date.")
				.show();
	}
	var n = r.charAt(2),
		l = '',
		c = oddt_day_text_value((a = dateFormatConvert(r, 'mm' + n + 'dd' + n + 'yy', '' + n, '/', a))),
	...
	if (
		...
		$.inArray('6', i.active_addon) > -1 && $('.identixweb-order-delivery-deliveryTime').html(l),
	...
	if ($.inArray('3', o) > -1 && 1 == pcAppStatus)
		$.get('/cart.json', '', function (e) {
			if (0 == oddProductAppStatus) {
				...
				if (void 0 === l.OrderDeliveryDate) {
					...
					for (var p = JSON.parse(l.OrderDeliveryDate), u = [], _ = 0; _ < p.length; _++)
						u[_] = p[_].variant_id;
					if (0 == u.includes(t)) {
		...
		$.ajax({ url: '/cart', data: u, type: 'POST', dataType: 'text', async: !1 }),
	...
	'onload' == e &&
		...
		($.inArray('3', o) > -1 && 1 == pcAppStatus
			? ($('.identixwebproductorderdeliverydeliveryTime').val(
					$('.identixwebproductorderdeliverydeliveryTime').find('option:eq(1)').val()
			  ),
			  $('.identixwebproductorderdeliverydeliveryTime').trigger('change'))
			: $.inArray('6', o) > -1
			? ($('.identixweb-order-delivery-deliveryTime').val(
					$('.identixweb-order-delivery-deliveryTime').find('option:eq(1)').val()
			  ),
...
function bindDatePicker(e, t, a, r, i, d, o, s, n) {
	...
	if (
		($('.uiDeliveryCalendar').datepicker({
			onChangeMonthYear: function (e, t, a) {
				...
				if (
					...
					var i = '01/' + (t > 9 ? '' : '0') + t + '/' + e,
						d = null != oddresultJson.location_id ? oddresultJson.location_id : 'IS NULL',
						s = 0;
						...
						(currentRequestoformonthchnage = $.ajax({
							...
							success: function (e) {
								(addonOrderData = JSON.parse(e)),
									...
									(oddresultJson.all_orders_status = addonOrderData.all_orders_status),
			...
			onSelect: function (e) {
				...
				var t = $(this).closest('.uiDeliveryCalendar').parents('.icartOddMainDiv');
				$('.icartOddMainDiv').length > 0 && t.length > 0 && icartCountClick($(this)),
					(l = 0),
		...
		$('form[action*="/cart"] [name="checkout"],form[action="/cart?locale=en"] [name="checkout"]').removeAttr(
			'disabled'
		),
		'squished-orders.myshopify.com' == shopName &&
			$('form[action*="/cart"] [name="checkout"],form[action="/cart?locale=en"] [name="checkout"]').show(),
		1 == oddresultJson.cal_type &&
		...
		1 === oddEditdateProductStatus &&
			($('.uiDeliveryCalendar').datepicker('setDate', oddEditdateProductDeliveryDate), 1 == n.timer_status))
	) {
		_ = r.charAt(2);
	...
	if (
		...
		0 == map_load &&
			1 == $('.store-pickup-identixweb-order-delivery-date').hasClass('active') &&
			null != oddresultJson.google_map_status &&
			1 == oddresultJson.google_map_status)
	)
		var S = setInterval(function () {
			$('#oddGoogleMap').length > 0 && (initMap(oddresultJson), clearInterval(S));
		}, 1e3);
}
...
function oddt_delivery_time_change_action(e, t) {
	var a = '.deliveryTime';
	$.inArray('6', oddresultJson.active_addon) > -1 && (a = '.identixweb-order-delivery-deliveryTime'),
		$(document).on('change', a, function () {
			$(a + ' option[value="' + $(this).val() + '"]').attr('selected', 'selected'),
			...
			if ('' != r) {
				...
				$.ajax({ url: '/cart', async: !0, data: s, type: 'POST', dataType: 'text' }), (cartParsData = s);
			}
			'1' == t && $('.additional-checkout-buttons, .dynamic-checkout__content').show(),
				'' == r && $('.additional-checkout-buttons, .dynamic-checkout__content').hide();
		});
}
...
function oddt_time_dropdown_format_zero(e, t, a, r) {
	...
	for (var g = 0; g < i.length; g++) {
		if (t == s && parseInt(n) == parseInt(_) && l == v) {
			if (0 == p) {
				if (parseFloat(c) <= parseFloat(i[g])) m += '<option value="' + f[g] + '">' + f[g] + '</option>';
			} else if (parseFloat(c) <= parseFloat(i[g]) && parseFloat(c) <= parseFloat(u))
...
function oddt_time_dropdown_format_two(e, t, a, r) {
	...
	if (
		...
		for (var g = 0; g < o.length; g++)
			(temp = '<option value="' + o[g] + '">' + o[g] + '</option>'), (h += temp);
		h = f + h;
	}
	return h;
}
function oddt_time_dropdown_format_three(e, t, a, r) {
	...
	var m,
		...
		g = [],
		D = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Special'],
		b = [],
		S = [],
		x = [],
		T = [],
	...
	for (w = 1 == w ? 'start' : 'end', B = 0; B < _.length; B++)
		if (i == D[B])
			for (m = 0; m < _[B].end.length; m++) {
				if (((y = _[B].end[m].split(':')), (temp_start = _[B].start[m].split(':')), 12 == v))
				...
				if (t == o && parseInt(s) == parseInt(h) && n == f)
					if (0 == c)
						if ('start' == w && 1 == e.hide_slots_settings) {
							if (parseFloat(l) <= parseFloat(temp_start[0]) && parseFloat(l) <= parseFloat(y[0])) {
								var N = A + '-' + J,
									R = L + ' ' + A + ' ' + P + ' ' + J + ' ' + O;
								b.push(N),
									S.push(R),
						...
						if (parseFloat(e.currentHr) <= parseFloat(y[0]) && parseFloat(e.currentHr) <= parseFloat(p)) {
							(N = A + '-' + J), (R = L + ' ' + A + ' ' + P + ' ' + J + ' ' + O);
							b.push(N),
								S.push(R),
								$.inArray('7', e.active_addon) > -1 &&
	...
	if (
		...
		$.inArray('7', e.active_addon) > -1 && (g.push(x.toString().split(',')), g.push(T.toString().split(','))),
		'' != g[0])
	) {
		var V = '<option value="">' + u + '</option>';
		if ($.inArray('7', e.active_addon) > -1 && 0 == pcAppStatus) {
			if ('' == $('.uiDeliveryDateVal').val() || null == $('.uiDeliveryDateVal').val()) var E = e.min_date;
			else E = $('.uiDeliveryDateVal').val();
			var M = order_limit_time_format(e, E, i, _, e.all_orders_status, e.all_orders, g[0], g[1], g[2], g[3]);
			...
			for (var U = 0; U < M[1].length; U++)
				void 0 !== M[0][U] && (V += '<option value="' + M[0][U] + '">' + M[1][U] + '</option>');
		} else
			for (var B = 0; B < g[1].length; B++) V += '<option value="' + g[0][B] + '">' + g[1][B] + '</option>';
		g = V;
	}
	return g;
}
		...
		if (c.length) {
			...
			for (var h = 0; h < o.length; h++)
				1 == n[h] &&
					...
					'' != timer && timer >= l[h] && (delete o[h], delete s[h], delete n[h], delete l[h]));
		...
		$.get('/cart.json', '', function (result) {
			...
			if (conditional_cart_value.length > 0) {
				...
				for (i = 0; i < conditional_cart_value.length; i++) {
					...
					if (cart_condition) {
						if (
							((oddshipping_price = parseFloat(scp_price)), (oddshipping_method = scp_method), 1 == status)
						) {
							shipping_call = 1;
							var price = storeCurrency + '' + oddshipping_price;
							(oddconditional_cart_apply = 1),
								(cart_message = replaceAll(cart_message, '{shipping.price}', price)),
								(cart_message = replaceAll(cart_message, '{shipping.method_name}', oddshipping_method)),
								$('.oddt_shipping_addon').css('display', 'flex'),
								$('.oddt_shipping_addon').html(
									'<span class="oddtDeliveryShippingPrice">' + cart_message + '</span>'
								);
						}
						return !1;
					}
			...
			if (conditional_shipping.length > 0) {
				...
				for (
					...
					var cs_price = conditional_shipping[i].cs_price,
						...
						condition = eval(parseInt(difference_In_Days) + operator + parseInt(day_name)),
						cs_price = parseFloat(cs_price);
					if (condition) {
						if (
							((oddshipping_price =
								1 == base_price ? parseFloat(cs_price) : parseFloat(cs_price) + dayWiseShippingRate),
							...
							(message = replaceAll(message, '{shipping.price}', price)),
								(message = replaceAll(message, '{shipping.method_name}', oddshipping_method)),
								$('.oddt_shipping_addon').css('display', 'flex'),
								$('.oddt_shipping_addon').html(
			...
			if ('' != oddshipping_price && 0 != oddshipping_price && 1 == status) {
				...
				(message = replaceAll(message, '{shipping.price}', price)),
					(message = replaceAll(message, '{shipping.method_name}', oddshipping_method)),
					$('.oddt_shipping_addon').css('display', 'flex'),
					$('.oddt_shipping_addon').html('<span class="oddtDeliveryShippingPrice">' + message + '</span>');
...
function doRequired(e, oddresultJson, requiredText, cartForm) {
	$('.deliveryRequired').hide(),
		cutofftime_with_no_show_time_select_box == shopName && (oddresultJson.timer_status = '0');
	...
	if (
		($.inArray('2', activeAddon) > -1 &&
			...
			(cartForm.removeAttr('action'), $('button[name="checkout"]').removeAttr('type')),
		1 == ldAppStatus &&
		...
		return (
			...
			$('.identixweb-order-delivery-date-deliveryRequired').html(localDeliveryMessage).show(),
			$('.order-delivery-date-postalcode-text').css({ border: '1px solid #ff0000', 'border-radius': '4px' }),
			void e.preventDefault()
		);
	var calender_class_name = '.ui-datepicker-inline';
	if (
		(1 == oddresultJson.cal_type && (calender_class_name = '.uiDeliveryCalendar'),
		'utrucking.myshopify.com' != shopName ||
			(deliveryDate &&
				(1 != timerStatus || deliveryTime) &&
				'' != cartForm.find("[name='attributes[Building]']").children('option:selected').val() &&
		...
		if (!deliveryDate || (1 == timerStatus && !deliveryTime))
			$('.identixweb-order-delivery-date-deliveryRequired').show(),
				$('html, body').animate({ scrollTop: $('.uiDeliveryCalendar').offset().top - 100 }, 1e3),
				$('.deliveryRequired').html(requiredText).show(),
				$.inArray('6', activeAddon) > -1 &&
					$('.identixweb-order-delivery-date-deliveryRequired').html(requiredText).show(),
				deliveryDate
				...
				'1' == timerStatus &&
					(deliveryTime
						? ($('.deliveryTime').removeClass('uiRequired'),
						  $('.identixweb-order-delivery-deliveryTime').removeClass('uiRequired'))
						: ($('.deliveryTime').addClass('uiRequired'),
						  $('.identixweb-order-delivery-deliveryTime').addClass('uiRequired'))),
				setTimeout(function () {
			...
			if (1 == oddconditional_cart_apply)
				var final_condition = eval(null != oddshipping_price || 0 == oddshipping_price);
			else var final_condition = eval('' != oddshipping_price && 0 != oddshipping_price);
			final_condition &&
				(e.preventDefault(),
				$.get('/cart.json', '', function (e) {
					...
					(t +=
						...
						$.ajax({
							type: 'POST',
							url: target + '?shop=' + shopName + '&method_name=oddt_draft_order',
							dataType: 'json',
							async: !0,
							data: t,
							success: function (e) {
			...
			cartForm.unbind('submit'),
				cartForm.find("[name='checkout']").length
					? (cartForm.find("[name='checkout']").unbind('click'), cartForm.find("[name='checkout']").click())
					: cartForm.submit();
	else
...
if (0 == mode);
	...
	(data.shop = Shopify.Checkout.apiHost),
		...
		window.Checkout.$.get(target, data, function (e) {
			...
			if (
				((e = JSON.parse(e)), (html = e.html), 'undefined' !== e.order.html && 1 == e.order.product_calendar)
			)
				'undefined' !== e.order_details &&
					1 == e.order.show_on_thankyou &&
					window.Checkout.$('.content-box:first').after(html);
			else if ('undefined' !== e.html && 1 == e.order_details.show_on_thankyou) {
...
$(document).ready(function () {
	$('.icart .icartOddMainDiv').length > 0 &&
		...
		oddJsLoad(oddSiteFdUrl + 'assets/js/icart_support_odd.min.js', 'oddSupportiCartJs');
});
